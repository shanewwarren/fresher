...
pub fn ensure_image_built(config: &Config) -> Result<()> {
    ...
    // If no presets, use base image directly (no build needed)
    if config.docker.presets.is_empty() {
        println!("{}", "[Docker] Using base image (no presets)".dimmed());
        return Ok(());
    }
...
pub fn get_image_tag(presets: &[String]) -> String {
    if presets.is_empty() {
        "ghcr.io/anthropics/claude-code-devcontainer:latest".to_string()
    } else {
